...
The feature selection pipeline (detailed in Chapter 3) reduced the 163 engineered features to 9 optimal features for the Simple LSTM model, achieving 94.5\% dimensionality reduction while maintaining high predictive accuracy.

\subsection{Final Feature Distribution}
\label{subsec:final_features}

Table \ref{tab:feature_categories} shows the distribution of selected features across categories for the best-performing Simple LSTM model (9 features total).
...
\hline
Price Features & 4 & 44.4\% \\
Weather Features & 2 & 22.2\% \\
Market \& Demand & 1 & 11.1\% \\
Supply Factors & 1 & 11.1\% \\
Fuel Prices & 1 & 11.1\% \\
Temporal Features & 0 & 0.0\% \\
\hline
\textbf{Total} & \textbf{9} & \textbf{100\%} \\
\hline
\end{tabular}
\end{table}
...
\subsection{Simple LSTM - Multivariate (Baseline)}
\label{subsec:simple_lstm_multivariate}

The Simple LSTM baseline model incorporated 9 selected features through a streamlined single-layer architecture designed to balance predictive capability with regularization constraints. Data preprocessing employed RobustScaler fitted on the training set to handle outliers, with 70-15-15 train-validation-test splits maintaining temporal ordering to prevent data leakage.

The network architecture featured a single LSTM layer with 32 units using tanh activation and L2 regularization coefficient of 0.01 to penalize large weights, combined with recurrent dropout rate of 0.2 applied to recurrent connections between time steps to prevent overfitting in the temporal dimension. This was followed by BatchNormalization layer to stabilize training dynamics and enable higher learning rates. The architecture then incorporated Dropout regularization with 0.4 rate for strong regularization given the moderate dataset size, followed by a dense layer with 16 units using ReLU activation and matching L2 regularization coefficient of 0.01 for final non-linear transformation. Additional Dropout with 0.3 rate preceded the single-unit output layer producing next-day price predictions.

The model was compiled with Adam optimizer using learning rate of 0.001 with gradient clipping (clipnorm=1.0) to prevent exploding gradients, Huber loss function providing robustness to outliers through combined squared and absolute error penalties, and batch size of 32 balancing computational efficiency with gradient stability. Training employed 100 maximum epochs with two callback mechanisms: EarlyStopping monitoring validation loss with patience of 15 epochs and automatic restoration of best weights, and ReduceLROnPlateau dynamically reducing learning rate by factor of 0.5 when validation loss plateaued for 7 epochs, with minimum learning rate floor of 0.00001 to enable fine-tuned convergence.

This baseline model established the foundation architecture that was refined through feature selection and architectural optimization to produce the best-performing Simple LSTM model described in the following subsection.
...
The model's key advantage stemmed from aggressive feature selection reducing the initial 163 engineered features to only 9 carefully selected variables, eliminating redundancy and noise while preserving essential predictive signals. This minimal feature set captured critical price dynamics through historical lag features, weather patterns from key growing regions, and fuel cost indicators, enabling the simpler architecture to achieve superior generalization. Training employed Adam optimizer with batch size of 32, Huber loss function for outlier robustness, and early stopping with patience of 15 epochs triggered at epoch 52 out of 67 total training epochs when validation performance plateaued.
...
The Simple LSTM's success demonstrates that for agricultural price forecasting with moderate-sized datasets, architectural simplicity combined with aggressive feature selection produces superior results compared to complex multilayer bidirectional architectures. The model's ability to achieve 19.93\% test MAPE with only 9 features validates the principle of parsimony in machine learning, where simpler models with well-chosen inputs often generalize better than complex architectures processing high-dimensional feature spaces.

\subsection{Bidirectional LSTM Architecture}
\label{subsec:bidirectional_lstm}

The Bidirectional LSTM model employed a more sophisticated architecture designed to capture temporal patterns from both past and future directions within the lookback window. The model processed the same 9 selected features used by the Simple LSTM baseline, utilizing identical data preprocessing and train-validation-test splits to ensure fair comparison.
...
\textbf{Response:} ``The Simple LSTM achieved 19.93\% MAPE with R² of 0.8651, significantly outperforming traditional ARIMA (\textgreater{}50\% MAPE) and ARIMAX (88.80\% MAPE). LSTM's ability to capture non-linear relationships between weather, supply, and prices provides 78\% MAPE reduction compared to ARIMAX.''
...
\begin{itemize}
\item \textbf{Curse of Dimensionality:} Adding 35 features increased model parameters from 10K to 45K, requiring proportionally more training data for effective learning. With 2,017 observations, the standard multivariate model suffered from overfitting (train MAPE 13.51\%, test MAPE 25.88\% - 12.37 point gap).
...
The Simple LSTM resolved these issues through aggressive feature selection (9 features), optimal regularization (Dropout + L2), and streamlined single-layer architecture, achieving 19.93\% MAPE - substantially better than univariate (21.90\%) and vastly superior to standard multivariate (25.88\%) variants. The Bidirectional LSTM with 19 features achieved intermediate performance at 21.46\% MAPE.

This demonstrates that feature engineering success depends critically on finding the optimal balance between information content and model complexity, with the Simple LSTM's 9-feature configuration striking this balance most effectively for the 2,017-observation dataset.
