...
\textbf{1. Develop Machine Learning Models for Price Forecasting}

Develop and implement multiple forecasting models using different machine learning approaches to predict daily wholesale carrot prices. This objective encompasses collecting and preprocessing historical price data along with relevant meteorological, economic, and market variables from January 2020 to July 2025, and developing three distinct modeling approaches: ARIMA (AutoRegressive Integrated Moving Average) models for traditional time series analysis, LSTM (Long Short-Term Memory) neural networks in both univariate and multivariate configurations to capture complex temporal dependencies, and Random Forest regression models as an ensemble learning alternative. The multivariate models incorporate external factors including precipitation patterns from major growing regions, fuel price indicators, supply factors, and temporal features, with rigorous feature selection methods applied to identify the most relevant predictors.

\textbf{2. Evaluate and Compare Model Performance}

Conduct comprehensive evaluation and comparison of all developed models using multiple performance metrics to identify the most effective forecasting approach. This includes systematic assessment using MAPE (Mean Absolute Percentage Error) for percentage error measurement, MAE (Mean Average Error) for absolute error quantification, RMSE (Root Mean Square Error) for penalizing large prediction errors, and R\textsuperscript{2} scores for explained variance assessment. The evaluation framework also includes ablation studies to isolate the contribution of different feature categories, statistical validation through bootstrap confidence intervals and cross-validation techniques, and analysis of feature importance to understand which factors most strongly influence carrot prices.

\textbf{3. Build an AI Agent Using RAG System}

Develop a deployment-ready AI agent based on Retrieval-Augmented Generation (RAG) architecture that integrates the best-performing forecasting model with natural language processing capabilities. This agent provides an accessible interface through which farmers, traders, and policymakers can query price predictions and obtain analytical insights without requiring technical expertise. The system implements a vector database for storing prediction results, utilizes the Groq API for large language model integration, and features a Gradio-based web interface for natural language interaction, democratizing sophisticated forecasting technology for non-technical stakeholders across the agricultural value chain.
